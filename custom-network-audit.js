...
//C:\Users\Tatsiana_Sabel\AppData\Roaming\npm\node_modules\lighthouse\lighthouse-core\index.js
const Audit = require('lighthouse/lighthouse-core/audits/audit.js');
const {taskGroups} = require('lighthouse/lighthouse-core/lib/tracehouse/task-groups.js');
...
/**
  ...
  static async audit(artifacts, context) {
    ...
    const trace = artifacts.traces[MyCustomAudit.DEFAULT_PASS];

    const tasks = await MainThreadTasks.request(trace, context);
    const multiplier = settings.throttlingMethod === 'simulate' ?
      settings.throttling.cpuSlowdownMultiplier : 1;

    const executionTimings = MyCustomAudit.getExecutionTimingsByGroup(tasks);
//    const executionTimings = MyCustomAudit.getExecutionTimingsBySelf(tasks);
...
/*	const results = Array.from(executionTimings).map(([urlAttr, rawDuration]) => {
      const duration = rawDuration * multiplier;
      totalExecutionTime += duration;

      const categoryTotal = categoryTotals[urlAttr] || 0;
      categoryTotals[urlAttr] = categoryTotal + duration;
...
*/
    ...
    const tableDetails = MyCustomAudit.makeTableDetails(headings, results);
...
module.exports = CustomNetworkAudit;